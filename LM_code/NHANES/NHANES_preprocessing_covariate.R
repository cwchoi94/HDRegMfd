...
data.path = 'https://wwwn.cdc.gov/Nchs/Nhanes/2003-2004/'

Covariate = Covariate_C
...
## description: https://wwwn.cdc.gov/Nchs/Nhanes/2003-2004/DEMO_C.htm
demographic = read_xpt(paste0(data.path,'DEMO_C.XPT'))
...
BP = read_xpt(paste0(data.path,'BPX_C.XPT'))
...
BM = read_xpt(paste0(data.path,'BMX_C.XPT'))
...
## https://wwwn.cdc.gov/Nchs/Nhanes/2003-2004/HDL_C.htm, etc
## total, HDL
total = read_xpt(paste0(data.path,'L13_C.XPT'))
total = data.frame(total[c('SEQN','LBXTC','LBXHDD')])
colnames(total) = c('SEQN','Total.chol','HDL')
### LDL
LDL = read_xpt(paste0(data.path,'L13AM_C.XPT'))
LDL = data.frame(LDL[c('SEQN','LBDLDL','LBXTR')])
colnames(LDL) = c('SEQN','LDL','Triglyceride')
### merge
cholesterol = merge(total,LDL,by='SEQN')

Covariate = merge(Covariate,cholesterol,by='SEQN')
rm(total,LDL,cholesterol)


## plasma glucose
## https://wwwn.cdc.gov/Nchs/Nhanes/2003-2004/GLU_C.htm
glucose = read_xpt(paste0(data.path,'L10AM_C.XPT'))
...
## https://wwwn.cdc.gov/Nchs/Nhanes/2003-2004/DR1TOT_C.htm, etc
intake = read_xpt(paste0(data.path,'DR1TOT_C.XPT'))
intake2 = read_xpt(paste0(data.path,'DR2TOT_C.XPT'))
...
save(Covariate,file=paste0(data.path0,'covariate_C_org.Rdata'))
...
save(Covariate,file=paste0(data.path0,'covariate_D_org.Rdata'))


### merge

load(paste0(data.path0,'covariate_C_org.Rdata'))
Covariate_C = Covariate
load(paste0(data.path0,'covariate_D_org.Rdata'))
Covariate_D = Covariate

Covariate = rbind(Covariate_C,Covariate_D)
