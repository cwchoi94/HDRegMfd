...
# sourceCpp('./__sources/cpp/LM_GCV.cpp')
...
model1 = LM.GCV(data$X,data$Y,datanew$X,datanew$Y,Yspace,penalty,gamma,lambda.list,Xdim.max.list,R.list,phi)
Yhat = predict(model1,datatest$X)

model2 = LM.GCV(data$X,data$Y,datanew$X,datanew$Y,Yspace,penalty,gamma,lambda.list,Xdim.max.list,R.list,phi2)
Yhat2 = predict(model2,datatest$X)

model3 = LM.GCV(data$X,data$Y,datanew$X,datanew$Y,Yspace,penalty,gamma,lambda.list,Xdim.max.list,R.list,phi3)
Yhat3 = predict(model3,datatest$X)

model4 = LM.oracle.GCV(data$X,data$Y,datanew$X,datanew$Y,Yspace,proper.indices,Xdim.max.list)
