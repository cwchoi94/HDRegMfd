...
# weather data
weather.org = read.csv(paste0(data.path0,'weather.csv'),fileEncoding='CP949')[,c(3,4,6,8,12,14,20)]
...
sit1 = 1*sapply(situation,function(x){('박무' %in% x) | ('안개' %in% x) | ('연무' %in% x)}) # misty or cloudy
sit2 = 1*sapply(situation,function(x){('눈' %in% x) | ('진눈깨비' %in% x) | ('비' %in% x) | ('소나기' %in% x) | ('안개비' %in% x)}) # snowy or rainy
