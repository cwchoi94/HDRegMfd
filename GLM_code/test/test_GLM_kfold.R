...
model1 = GLM.kfold(data$X,data$Y,link,kfold,seed,penalty,gamma,lambda.list,Xdim.max.list,R.list,phi)
Yhat = predict(model1,datatest$X)

model2 = GLM.kfold(data$X,data$Y,link,kfold,seed,penalty,gamma,lambda.list,Xdim.max.list,R.list,phi2)
Yhat2 = predict(model2,datatest$X)

model3 = GLM.kfold(data$X,data$Y,link,kfold,seed,penalty,gamma,lambda.list,Xdim.max.list,R.list,phi3)
Yhat3 = predict(model3,datatest$X)

model4 = GLM.oracle.kfold(data$X,data$Y,link,proper.indices,kfold,seed,Xdim.max.list)
