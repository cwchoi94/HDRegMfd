...
for(k in 1:kfold){
  ...
  for (penalty in penalty.list){
    ...
    model = GLM.CV(Xtrain,Ytrain,link,proper.indices,cv.type,Xdim.max.list)
...
rownames(result) = c('accuracy','auc','specificity','sensitivity','ratio.train','ratio.test')
colnames(result) = c(penalty.list)
write.csv(result,paste0(result.path0,'wind_class_result_avg.csv'))
