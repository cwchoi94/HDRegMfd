...
result.path = paste0(result.path0,'quantile/')
...
for (Ycol in Ycols){
  ...
  for(k in 1:kfold){
    ...
    colnames(result) = c(penalty.list,'only PA')
  ...
  rownames(result) = c('accuracy','auc','specificity','sensitivity','ratio.train','ratio.test')
  colnames(result) = c(penalty.list,'only PA')
  write.csv(result,paste0(result.path,'class_result_avg_quantile_',Ycol,'.csv'))
}
...
for (Ycol in Ycols){
  result = read.csv(paste0(result.path,'class_result_avg_quantile_',Ycol,'.csv'))
...
write.csv(accuracy,paste0(result.path0,'Accuracy_quantile_all.csv'))
write.csv(AUC,paste0(result.path0,'AUC_quantile_all.csv'))
...
read.csv(paste0(result.path0,'Accuracy_quantile_all.csv'))
read.csv(paste0(result.path0,'AUC_quantile_all.csv'))
