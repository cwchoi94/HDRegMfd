...
scores = c('CS','ICS','RMSE','MSE','RMSE2','MSE2','accuracy','accuracy.true','CV.iteration','runtime.second','runtime.opt.second')
  ...
  for (beta.norm in beta.norm.list){
    ...
    for (iter in 1:n.iteration){
      ...
      if (!(iter %in% result.each[,'iter'])){
        ...
        RMSE = sqrt(MSE)
        RMSE2 = sqrt(MSE2)
        accuracy = mean(datatest$Y == Yclass)
        accuracy.true = mean(datatest$Y == 1.0 * (datatest$Ymu>=0.5))
        ...
        tmp.scores = list(iter,CS,ICS,RMSE,MSE,RMSE2,MSE2,accuracy,accuracy.true,CV.iteration,runtime.second,runtime.opt.second,model$lambda,model$Xdim.max,model$R)
        result.each = read.csv(paste0(result.name.base,'lasso','_',count,'.csv'))[,-1]
      ...
      if (!(iter %in% result.each[,'iter'])){
        ...
        tmp.scores = list(iter,CS,ICS,RMSE,MSE,RMSE2,MSE2,accuracy,accuracy.true,0,runtime.second,runtime.opt.second,0,model$Xdim.max,0)
        result.each = read.csv(paste0(result.name.base,'oracle','_',count,'.csv'))[,-1]
        result.each = rbind(result.each,tmp.scores)
        result.each = result.each[order(result.each[,'iter']),]
        result.each = result.each[!duplicated(result.each[,'iter']),]
    ...
    result.tmp = list(n,beta.norm,CS,ICS,RMSE,MSE,RMSE2,MSE2,accuracy,accuracy.true,CV.iteration,runtime.second,runtime.opt.second)
