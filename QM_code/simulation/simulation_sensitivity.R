...
# create save folder
save_path = paste0('./sim_result/sensitivity_',kernel,'_',cv.type,'_',n,'_',p,'_',s,'/')
...
loops = expand.grid(c.h=c.h.list,tau=tau.list)
n.count = nrow(loops)
...
for (error.type.idx in error.type.idx.list){
  ...
  for (count in 1:n.count){
    ...
    for (name in model.names){
      if (!file.exists(paste0(result.name.base.each,name,'_',tau,'_',c.h,'.csv'))){
        result.each = t(data.frame(rep(0,length(cols.each)),row.names=cols.each))
        rownames(result.each) = NULL
        write.csv(result.each,file=paste0(result.name.base.each,name,'_',tau,'_',c.h,'.csv'))
      }
    }
    ...
    for (iter in 1:n.iteration){
      ...
      model.run.check = sapply(model.names,function(name){
        result.each = read.csv(paste0(result.name.base.each,name,'_',tau,'_',c.h,'.csv'))[,-1]
        return(iter %in% result.each[,'iter'])
      })
      if (all(model.run.check)){next}
      
      message('model: ',base.model,', cv.type: ',cv.type,', n:',n, ', p: ',p,', s: ',s)
      message('count: ',count.all,'/',n.count.all,', error.type: ',error.type.org,', tau: ',tau,', c.h: ',c.h,', iter: ',iter,'/',n.iteration)
      ...
      # LASSO
      penalty = 'LASSO'
      result.each = read.csv(paste0(result.name.base.each,penalty,'_',tau,'_',c.h,'.csv'))[,-1]
      if (!(iter %in% result.each[,'iter'])){
        model = QM.CV(data$X,data$Y,tau,h,kernel,cv.type,penalty,gamma,lambda.list,Xdim.max.list,cv.const=cv.const,c.h=c.h)
        ...
        rownames(result.each) = NULL
        write.csv(result.each,paste0(result.name.base.each,penalty,'_',tau,'_',c.h,'.csv'))
      ...
      result.each = read.csv(paste0(result.name.base.each,penalty,'_',tau,'_',c.h,'.csv'))[,-1]
      if (!(iter %in% result.each[,'iter'])){
        model = QM.oracle.CV(data$X,data$Y,tau,h,kernel,proper.indices,cv.type,Xdim.max.list,cv.const=cv.const,c.h=c.h)
    ...
    # # LASSO
    # result.each = read.csv(paste0(result.name.base.each,'LASSO','_',tau,'_',c.h,'.csv'))[-1,]
    ...
    #   }
    # }
    # result.tmp = list(tau,c.h,TP,FP,RMSE,MSE,MAD,RMSPE,MSPE,runtime.second,runtime.opt.second)
    ...
    # 
    # # SCAD
    # result.each = read.csv(paste0(result.name.base.each,'SCAD','_',tau,'_',c.h,'.csv'))[-1,]
    # result.each = result.each[rowSums(is.na(result.each))==0,]
    # for (score in scores){
    #   score.mean = mean(result.each[[score]])
    #   score.se = sd(result.each[[score]])/sqrt(nrow(result.each))
    ...
    # 
    # # MCP
    # result.each = read.csv(paste0(result.name.base.each,'MCP','_',tau,'_',c.h,'.csv'))[-1,]
    # result.each = result.each[rowSums(is.na(result.each))==0,]
    # for (score in scores){
    #   score.mean = mean(result.each[[score]])
    #   score.se = sd(result.each[[score]])/sqrt(nrow(result.each))
    ...
    # 
    # # oracle
    # result.each = read.csv(paste0(result.name.base.each,'ORACLE','_',tau,'_',c.h,'.csv'))[-1,]
    # result.each = result.each[rowSums(is.na(result.each))==0,]
    # for (score in scores){
    #   score.mean = mean(result.each[[score]])
    #   score.se = sd(result.each[[score]])/sqrt(nrow(result.each))
