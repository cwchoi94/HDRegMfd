...
List LM_CV(List X, arma::mat LogY, arma::vec Ymu, String Yspace, arma::vec lambda_list, arma::vec Xdim_max_list, arma::vec R_list, String cv_type = "AIC",
           String penalty = "LASSO", double gamma = 0, double phi = 1, double cv_const = 2.0, int max_cv_iter = 20, double threshold = 1e-10) {
    ...
    while (iter < max_cv_iter) {
        ...
        if (r1 > 1 || iter==0) {
            for (int i = 0; i < r1; i++) {
                ...
                loss1(i) = get_loss_CV_LM(X, LogY, Ymu, Yspace, lambda, opt_Xdim_max, opt_R, cv_type, penalty, gamma, phi, cv_const);
            }
        }
        else {
            loss1(0) = loss_min;
        ...
        if (r2 > 1) {
            for (int i = 0; i < r2; i++) {
                ...
                loss2(i) = get_loss_CV_LM(X, LogY, Ymu, Yspace, opt_lambda, Xdim_max, opt_R, cv_type, penalty, gamma, phi, cv_const);
            }
        }
        else {
            loss2(0) = loss_min;
        ...
        if (r3 > 1) {
            for (int i = 0; i < r3; i++) {
                double R = R_list(i);
                loss3(i) = get_loss_CV_LM(X, LogY, Ymu, Yspace, opt_lambda, opt_Xdim_max, R, cv_type, penalty, gamma, phi, cv_const);
            }
        }
        else {
            loss3(0) = loss_min;
