...
arma::mat LR_betaj_update(arma::mat wj, double sigmaj, double normwj, String penalty, double lambdaj, double kappaj, double gamma, mat betaj, double betaj_norm){
    mat betaj_new;
...
List LR_each(List Xorg, arma::mat LogY, arma::vec Ymu, Function inner, double lambda, int Xdim_max, double R, double phi, String penalty, double gamma, double eta, int max_iter, double threshold){
    ...
    while (iter<max_iter){
        ...
        for (int j=0; j<p; j++){
            ...
            // betaj update
            mat betaj = LR_betaj_update(wj,sigmaj,normwj,penalty,lambdaj,kappaj,gamma,beta.rows(ind1),beta_norm(j));
...
double get_loss_LR(List X, arma::mat LogY, List Xnew_, arma::mat LogYnew, arma::vec Ymu, Function inner, double lambda, int Xdim_max, double R, double phi, String penalty, double gamma){
    ...
    // model training
    List model = LR_each(X,LogY,Ymu,inner,lambda,Xdim_max,R,phi,penalty,gamma);
    mat beta = model["beta"];
