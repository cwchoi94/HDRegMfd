...
// [[Rcpp::export]]
List LR_GCV(List X, arma::mat LogY, List Xnew, arma::mat LogYnew, arma::vec Ymu, Function inner, arma::vec lambda_list, arma::vec Xdim_max_list, arma::vec R_list, 
    ...
    while (iter<max_cv_iter){
        ...
        for (int i=0; i<r1; i++){
            ...
            loss1(i) = get_loss_LR(X,LogY,Xnew,LogYnew,Ymu,inner,lambda,opt_Xdim_max,opt_R,phi,penalty,gamma);
        }
        ...
        for (int i=0; i<r2; i++){
            ...
            loss2(i) = get_loss_LR(X,LogY,Xnew,LogYnew,Ymu,inner,opt_lambda,Xdim_max,opt_R,phi,penalty,gamma);
        }
        ...
        for (int i=0; i<r3; i++){
            double R = R_list(i);
            loss3(i) = get_loss_LR(X,LogY,Xnew,LogYnew,Ymu,inner,opt_lambda,opt_Xdim_max,R,phi,penalty,gamma);
        }
