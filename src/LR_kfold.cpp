...
// [[Rcpp::export]]
List LR_Kfold(List X_list, List LogY_list, List Xnew_list, List LogYnew_list, List Ymu_list, Function inner, int kfold, arma::vec lambda_list, arma::vec Xdim_max_list, arma::vec R_list, 
    ...
    while (iter<max_cv_iter){
        ...
        for (int idx=0; idx<kfold; idx++){
            ...
            for (int i=0; i<r1; i++){
                ...
                loss1_mat(idx,i) = get_loss_LR(X,LogY,Xnew,LogYnew,Ymu,inner,lambda,opt_Xdim_max,opt_R,phi,penalty,gamma);
            }
        }
        ...
        for (int idx=0; idx<kfold; idx++){
            ...
            for (int i = 0; i < r2; i++) {
                ...
                loss2_mat(idx, i) = get_loss_LR(X,LogY,Xnew,LogYnew,Ymu,inner,opt_lambda,Xdim_max,opt_R,phi,penalty,gamma);
            }
        }
        ...
        for (int idx=0; idx<kfold; idx++){
            ...
            for (int i = 0; i < r3; i++) {
                double R = R_list(i);
                loss3_mat(idx, i) = get_loss_LR(X,LogY,Xnew,LogYnew,Ymu,inner,opt_lambda,opt_Xdim_max,R,phi,penalty,gamma);
            }
        }
