    ...
    while (iter < max_cv_iter) {
        ...
        if (r1 > 1 || iter==0) {
            for (int i = 0; i < r1; i++) {
                ...
                loss1(i) = get_loss_LM(X, LogY, Xnew, LogYnew, Ymu, Yspace, lambda, opt_Xdim_max, opt_R, penalty, gamma);
            }
        }
        else {
            loss1(0) = loss_min;
        ...
        if (r2 > 1) {
            for (int i = 0; i < r2; i++) {
                ...
                loss2(i) = get_loss_LM(X, LogY, Xnew, LogYnew, Ymu, Yspace, opt_lambda, Xdim_max, opt_R, penalty, gamma);
            }
        }
        else {
            loss2(0) = loss_min;
        ...
        if (r3 > 1) {
            for (int i = 0; i < r3; i++) {
                double R = R_list(i);
                loss3(i) = get_loss_LM(X, LogY, Xnew, LogYnew, Ymu, Yspace, opt_lambda, opt_Xdim_max, R, penalty, gamma);
            }
        }
        else {
            loss3(0) = loss_min;
