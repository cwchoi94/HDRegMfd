    ...
    while (iter<max_cv_iter){
        ...
        if (r1 > 1 || iter == 0) {
            for (int idx = 0; idx < kfold; idx++) {
                ...
                for (int i = 0; i < r1; i++) {
                    ...
                    loss1_mat(idx, i) = get_loss_LM(X, LogY, Xnew, LogYnew, Ymu, Yspace, lambda, opt_Xdim_max, opt_R, penalty, gamma, phi);
                }
            }
        }
        else {
        ...
        if (r2 > 1) {
            for (int idx = 0; idx < kfold; idx++) {
                ...
                for (int i = 0; i < r2; i++) {
                    ...
                    loss2_mat(idx, i) = get_loss_LM(X, LogY, Xnew, LogYnew, Ymu, Yspace, opt_lambda, Xdim_max, opt_R, penalty, gamma, phi);
                }
            }
        }
        else {
        ...
        if (r3 > 1) {
            for (int idx = 0; idx < kfold; idx++) {
                ...
                for (int i = 0; i < r3; i++) {
                    double R = R_list(i);
                    loss3_mat(idx, i) = get_loss_LM(X, LogY, Xnew, LogYnew, Ymu, Yspace, opt_lambda, opt_Xdim_max, R, penalty, gamma, phi);
                }
            }
        }
        else {
