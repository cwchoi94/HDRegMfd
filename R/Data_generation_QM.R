...
#' @param error.var.type a type of error distribution, one of 'homoscedastic' or 'heteroscedastic'.
...
QM.error.generate = function(n,Xdata,tau=0.5,error.std=1,error.var.type='homoscedastic',error.type='normal',df=3){
  
  Yspace = 'Euclid'
  Ymu = 0
  
  if (error.var.type=='homoscedastic'){
    error.X.std = rep(1,n) * error.std
  } else if (error.var.type=='heteroscedastic'){
    ...
    stop("The 'error.var.type' must be one of 'homoscedastic' or 'heteroscedastic'.")
  }
...
#' @inheritParams QM.error.generate
#' 
...
#' }
#' @export
QM.data.generate = function(n,Xspaces,Xdims,tau=0.5,proper.indices=NULL,beta.norm=1,beta0.norm=1,Xrho=0.5,Xsigma=1,error.std=1,error.var.type='homoscedastic',error.type='normal',df=3,ngrid=100,seed=1,c.beta=NULL){
