...
LR.GCV = function(Xorg,Yorg,Xorgnew,Yorgnew,Yspace,lambda.list,Xdim.max.list,R.list,phi=1,penalty='LASSO',gamma=0,
  ...
  # Use LR_GCV function to obtain the optimal parameters
  result = LR_GCV(X,LogY,Xnew,LogYnew,Ymu,inner,lambda.list,Xdim.max.list,R.list,
  ...
  # apply LR with the optimal parameters
  opt.lambda = result$opt.lambda
  opt.Xdim.max = result$opt.Xdim.max
  opt.R = result$opt.R
  
  object = LR_each(X,LogY,Ymu,inner,opt.lambda,opt.Xdim.max,opt.R,phi,penalty,gamma,eta,max.iter,threshold)
